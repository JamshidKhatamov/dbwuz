...
if (db.has(`prefix_${message.guild.id}`) === false) {
  ...
  if(db.fetch(`yasakK_${message.guild.id}`)) return message.channel.send('Bu komut bu sunucuda **yasaklanmıştır!**')
  
}
  let veri = await db.fetch(`botbakım`)
if(veri) {
 if(message.author.id !== "469111838295064586") {
 ...
 .setColor('RED')
message.channel.send(codeming).then(m => m.delete({ timeout: 10000 }))
 return
 } 
...
// .setDescription(`**${message.guild.name}** Adlı sunucuda **${message.author.tag}** adlı kullanıcı **${cmd.help.name}** komutunu kullandı.`)
  //client.guilds.cache.get('627204002676736031').channels.cache.get('657976342989766677').send(embed22)
  
// if (db.has(`yasak_${message.guild.id}_${cmd.help.name}`)){
 //   return message.channel.send(client.emojis.cache.get(client.emojiler.hayır) + ' Bu komut sunucuda **yasaklanmıştır!**')
// }
// if (db.has(`yasak_${message.guild.id}_${cmd.help.name}`)){
 //   return message.channel.send(client.emojis.cache.get(client.emojiler.hayır) + ' Bu komut sunucuda **yasaklanmıştır!**')
// }
...
db.add(`komutklnm_${message.author.id}`, 1)
  ...
  if (cmd) {
    ...
    if (db.has(`karalist_${message.author.id}`) === true) {
      if (!client.users.cache.get('469111838295064586')) {
...
.then(message => message.delete({ timeout: 10000 }));
     ...
     if(db.has(`üyelikk_${message.member.id}`)) {
if(message.channel.id !== sunucu) return message.channel.send(`<a:gif2:791583030200238110>Kral, <@${message.author.id}> Burası komut kanalı değil. <#`+ sunucu+`> kanalına giderek komutlarımı ordan kullanabilirsin. <a:gif2:791583030200238110>`)
.then
(message.delete())    
.then(message => message.delete({ timeout: 10000 }));
  }
